    ...
    _calculatorPoint(pointConfig, recordValue) {
        ...
        for (var i = count - 1; i >= 0; i--) {
            ...
            if ((pointConfig.record_unit === me._POINT_UNIT._TIME && recordValueCal < item.record_value_cal)
                ...
                //tình huống không có offset thì lấy bằng giá trị mốc này luôn, không có tính toán
                if (item.record_offset === 0 || !item.record_offset) {
                    return item.point_value;
                }
    ...
    _convertValue(unit, recordValue) {
        ...
        switch (unit) {
            case me._POINT_UNIT._TIME:
                return me._convertValue_TIME(String(recordValue));
                break;
            case me._POINT_UNIT._NUMBER:
            default:
                return parseFloat(String(recordValue).replace(',', '.'));
        }
